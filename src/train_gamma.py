...
def run_training_session(hyperparams, train_loader, val_loader, args, trial=None):
    ...
    with open(log_file_path, "w") as log_file:
        log_file.write(
            "epoch,"
            "train_loss_total,train_loss_A,train_loss_P,train_loss_CC,"
            "val_loss_total,val_loss_A,val_loss_P,val_loss_CC\n"
        )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for input_data, log_target_gamma, _, _ in train_loader:
            ...
            if current_arch == "Constrained" or current_arch == "Lipschitz":
                WEIGHT_A = 3.0
                WEIGHT_P = 1.0
                WEIGHT_CC = 1.5
            else:
            ...
            weighted_total_loss, loss_A, loss_P, loss_CC = criterion(
                predicted_gamma_log, log_target_gamma, WEIGHT_A, WEIGHT_P, WEIGHT_CC
            )
        ...
        with open(log_file_path, "a") as log_file:
            log_file.write(
                f"{epoch+1},"
                f"{avg_train['total']:.6f},{avg_train['loss_A']:.6f},{avg_train['loss_P']:.6f},{avg_train['loss_CC']:.6f},"
                f"{avg_val['total']:.6f},{avg_val['loss_A']:.6f},{avg_val['loss_P']:.6f},{avg_val['loss_CC']:.6f}\n"
            )
