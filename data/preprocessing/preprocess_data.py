    ...
    for task in tasks:
        ...
        try:
            ...
            if 'y' not in ds.dims or 'x' not in ds.dims:
                results.append(f"Skipped index {idx}: Source {source_folder} dimensions missing.")
                continue
            ...
            if h != patch_size or w != patch_size:
                results.append(f"Skipped patch {patch_meta} at index {idx}: boundary truncation to shape ({h}, {w})")
                continue
            ...
            if dh != patch_size or dw != patch_size:
                results.append(f"Skipped patch {patch_meta} at index {idx}: DEM boundary truncation.")
                continue
            ...
            filtered_precip = filter_precip_bounds(original_precip, drizzle_thresh, declutter_thresh)
            ...
            target_zarr[f"{group_name}/original_precip"][idx] = filtered_precip.astype(np.float32)
            target_zarr[f"{group_name}/interpolated_precip"][idx] = interpolated.astype(np.float32)
...
def compute_global_scaler(zarr_path, config):
    ...
    for i in tqdm(range(0, train_data.shape[0], chunk_size), desc="Scanning Train Data"):
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess patch data in parallel from metadata files.")
    ...
    output_zarr_path = os.path.join(config["PREPROCESSED_DATA_DIR"], "preprocessed_dataset.zarr")
    ...
    for group_name, batched_payloads in all_batched_payloads.items():
        ...
        with ProcessPoolExecutor(max_workers=config["MAX_WORKERS"]) as executor:
            futures = [executor.submit(process_batch, payload) for payload in batched_payloads]
            for future in tqdm(as_completed(futures), total=len(batched_payloads), desc=f"Processing {group_name}"):
